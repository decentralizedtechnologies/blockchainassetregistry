import { Backdrop, Box, Button, Container, Grid, Paper, Theme, Typography, withStyles } from "@material-ui/core";
...
import { enableEthereumWallet, ERC20, getClient, isEthereumEnabled, isWeb3Compatible, networkNameByVersion, Networks, networks, setClientProviderByNetwork } from "../../client";
...
export const Create = withStyles((theme: Theme) => ({
  ...
  const [isWalletEnabledError, setIsWalletEnabledError] = React.useState(false);
  ...
  const onEnableEthereumClient = async () => {
    try {
      setIsWalletEnabledError(false);
      await enableEthereumWallet();
      setTimeout(async () => {
        try {
          setIsWalletEnabled(await isEthereumEnabled());
        } catch (error) {
          setIsWalletEnabledError(true);    
        }
      }, 1500);
    } catch (error) {
      setIsWalletEnabledError(true);
                ...
                <>
                  {!isWalletEnabled ? (
                    <>
                      ...
                      {isWalletEnabledError && (
                        <Box mt={1}>
                          <Typography variant="body2" className={classes.error}>
                            An error ocurred while connecting your wallet. Are you in a HTTPs secure
                            connection?
                          </Typography>
                        </Box>
                      )}
